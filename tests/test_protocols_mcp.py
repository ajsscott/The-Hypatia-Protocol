...
- list_resources returns all 36 expected resources (20 protocols + 16 kernel-archive)
...
EXPECTED_ARCHIVE_URIS = {
    ...
    # Generated by scripts/split-decision-routes.py from 11-decision-routes.md
    "protocol://detail/decision-route-a",
    "protocol://detail/decision-route-b",
    "protocol://detail/decision-route-c",
    "protocol://detail/decision-route-d",
    "protocol://detail/decision-route-e",
    "protocol://detail/decision-route-f",
}
...
class TestProtocolsMCPServer(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            # Binary exists but was built for another platform (e.g. a macOS
            # binary checked into target/ while tests run on Linux CI).
            raise unittest.SkipTest(f"cannot execute {BINARY}: {e}")
...
class TestServedFilesExist(unittest.TestCase):
    ...
    def test_generated_route_files_current(self):
        """The per-route files must exist and match their generator's output."""
        result = subprocess.run(
            [sys.executable, str(REPO_ROOT / "scripts" / "split-decision-routes.py"), "--check"],
            capture_output=True,
            text=True,
        )
