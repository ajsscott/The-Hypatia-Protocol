...
** boundary**: Hypatia does NOT write to these stores during routine sessions. New entries go to `inbox/preferences/*.md`; the Scholar consolidates manually during maintenance. Direct writes to these files are exceptional and require Tier 1 destructive confirmation.
...
### Rule 1: Intelligence stores follow 

Hypatia writes to `inbox/preferences/*.md`, not to `patterns.json` / `knowledge.json` / `reasoning.json` / `cross-references.json`. The Scholar consolidates from inbox to stores during maintenance. Any direct write to the stores by Hypatia during a session is a boundary violation; treat as Tier 1 destructive.
...
- [ ] Verify the operation doesn't violate boundary (inbox-only for routine writes to stores).
...
*This protocol exists because intelligence stores represent accumulated learning that cannot be recreated. The inbox boundary exists for the same reason: review-before-canon is the Scholar's hedge against silent corruption.*
