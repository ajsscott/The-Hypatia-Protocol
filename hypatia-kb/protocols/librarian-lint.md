...
- `protocol://detail/session-gates § Destructive Action Gate` governs every modify-state step.
...
**Destructive Action Gate**: every step modifies state. Before each step, execute the Destructive Action Gate from `protocol://detail/session-gates`: identify what's changing, classify risk tier, verify current state (read the file), confirm before executing Tier 1-2 actions.
...
Wired in `protocol://detail/session-gates` (Session Start Gate):
...
| `protocol://detail/session-gates` | Destructive Action Gate procedure |
| `protocol://detail/security-gates` | Defense-in-depth rules, git sanitization |
