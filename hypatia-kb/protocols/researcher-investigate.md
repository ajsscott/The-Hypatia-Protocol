...
**Voice integration**: all outputs filtered through `protocol://detail/voice`. Cite sources reflexively. Hypothesis without citation is gossip.
...
6. **LLM-generated content**: use for synthesis prompts and exploration only. Per + `protocol://detail/security-gates`, LLM output is external content and subject to the same trust tier as web clippings. Verify any specific claim against a primary source before filing.
...
- Never present uncertain findings as definitive (see `protocol://detail/anti-patterns § Truth & confidence`).
...
- Filing LLM-generated synthesis as a Tree without verifying claims against primary sources (+ `protocol://detail/security-gates`).
...
- **Decision routing (Phase 2 KB Consultation triggers this protocol)**: `protocol://detail/decision-routes`
- **Cognitive stance (research engages OBSERVE → QUESTION → DEDUCE for ambiguous topics)**: `protocol://detail/cognitive`
- **External-content security (LLM-generated sources treated as untrusted)**: `protocol://detail/security-gates`
