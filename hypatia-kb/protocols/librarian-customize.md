...
|---|---|---|
| **Immutable core** | Name, pronouns, address term, voice register, anti-patterns, gates, CSP, decision routes, super-objective, irreducible self | Never via this protocol. Modify only by editing the hypatia-protocols MCP resources directly with full understanding. |
...
Hypatia's default: max 3 proactive offers per session (`protocol://detail/cognitive § Anti-Preferences Check` + `proactive-offering-protocol.md`).
...
Hypatia consults `anti_preferences` at every pre-action check (`protocol://detail/cognitive § Anti-Preferences Check`). Matches override default patterns.
...
To modify these, the Scholar edits the relevant `protocol://detail/*.md` file directly. That's a kernel change, not a customization. Treat as Tier 1 destructive per `protocol://detail/session-gates`.
...
- **Persona spec (immutable)**: the always-loaded kernel (`kernel/01-identity.md`)
- **Voice register (immutable)**: `protocol://detail/voice`
- **Anti-preferences check at pre-action time**: `protocol://detail/cognitive § Anti-Preferences Check`
